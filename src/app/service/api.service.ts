...
export class ApiService {
  ...
  _addPins(payload: any) {
    if (Object.keys(payload).length > 0) {
      this.allPins.push(payload);
      return of({
        status: true,
        data: payload,
  ...
  _addCustomer(payload: any) {
    if (Object.keys(payload).length > 0) {
      this.allCustomer.push(payload);
      return of({
        status: true,
        data: payload,
